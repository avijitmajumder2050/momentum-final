...
def execute_trade(
    ...
    log.info("[Executor] REAL ENTRY=%s QTY=%s", real_entry, real_qty)
...
def fetch_entry_from_position(broker, symbol: str, token: str, retries: int = 8, delay: float = 1.5):
    import time
    ...
    for i in range(retries):
        ...
        if res and res.get("status") == True:
            for p in res.get("data", []):
                if p.get("symboltoken") == str(token) or p.get("tradingsymbol") == symbol:

                    qty = int(p.get("netqty", 0))

                    # IMPORTANT: don't skip immediately
                    if qty == 0:
                        continue

                    entry = float(p.get("buyavgprice") or p.get("avgnetprice"))
                    return entry, qty

        time.sleep(delay)

    raise Exception(f"Position not found after retries for {symbol}")