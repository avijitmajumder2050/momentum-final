"""
trailing_engine.py
==================
Continuous trailing stop-loss engine.

Run cycle (every TRAIL_POLL_SECS seconds)
-----------------------------------------
For each ACTIVE trade:

  1. Fetch live LTP from Angel API (never from CSV).

  2. PHASE 1 — BREAKEVEN  (fires once)
     Condition : LTP >= entry * (1 + BREAKEVEN_PCT)   default +1%
     Action    : Move SL to entry price
     SubAction : BREAKEVEN

  3. PHASE 2 — TRAILING   (fires repeatedly)
     step          = max(1, round(ltp * TRAIL_STEP_PCT))  default 0.2%
     candidate_sl  = ltp - step
     Condition : candidate_sl > Last_SL (only move UP, never down)
     Action    : Modify GTT SL via Angel API → update Last_SL in CSV
     SubAction : TRAILING

  4. TARGET CHECK
     Condition : LTP >= Target_Price
     Action    : close_trade (TARGET_HIT)

  5. SL CHECK
     Condition : LTP <= Last_SL
     Action    : close_trade (SL_HIT)

  6. TIME EXIT
     Condition : Entry_Time + TIME_EXIT_MINS elapsed AND Status == ACTIVE
     Action    : cancel GTT, place market SELL, close_trade (TIME_EXIT)

Idempotency
-----------
- SL only moves UP — a lower candidate_sl is always ignored.
- Trailing flag is set True in CSV so restart-safe.
- All operations are wrapped in try/except per trade — one bad symbol
  never blocks others.

SubAction values written to CSV
---------------------------------
ENTRY / BREAKEVEN / TRAILING / TARGET_HIT / SL_HIT /
MANUAL_CANCEL / TIME_EXIT
"""
import os
import time
import logging
import threading
from datetime import datetime, timedelta
from typing import Optional

from trade_s3 import load_active, update_trailing_sl, close_trade, update_trade

log             = logging.getLogger(__name__)
TRAIL_POLL_SECS = int(os.getenv("TRAIL_POLL_SECS","10"))
BREAKEVEN_PCT   = float(os.getenv("BREAKEVEN_PCT", "0.01"))   # 1% above entry
TRAIL_STEP_PCT  = float(os.getenv("TRAIL_STEP_PCT","0.002"))  # 0.2% of LTP
TIME_EXIT_MINS  = int(os.getenv("TIME_EXIT_MINS",  "10"))


def calc_step(ltp: float) -> float:
    """Dynamic trail step: 0.2% of LTP, minimum ₹1."""
    return max(1.0, round(ltp * TRAIL_STEP_PCT, 2))


def _ist_now() -> datetime:
    import pytz, datetime as dt
    return dt.datetime.now(pytz.timezone("Asia/Kolkata"))


def _market_open() -> bool:
    n    = _ist_now()
    mins = n.hour * 60 + n.minute
    return n.weekday() < 5 and 555 <= mins <= 930


def process_trade(broker, trade: dict) -> None:
    """
    Run the full trailing + exit logic for one ACTIVE trade.
    Called once per poll cycle.  All errors are caught and logged.
    """
    order_id = trade["Order_ID"]
    symbol   = trade["Symbol"]
    token    = trade["Angel_Token"]

    try:
        # ── Live LTP (never from CSV) ─────────────────────────────────────────
        ltp = broker.get_ltp("NSE", symbol, token)
    except Exception as e:
        log.warning("[Trail] LTP failed for %s: %s", symbol, e)
        return

    try:
        entry  = float(trade["Entry_Price"])
        target = float(trade["Target_Price"])
        last_sl = float(trade["Last_SL"] or trade["SL_Price"])
        qty    = int(trade["Qty"])
        gtt_sl_id = trade.get("GTT_SL_ID","")
    except (ValueError, TypeError) as e:
        log.error("[Trail] Parse error for %s: %s", symbol, e)
        return

    # ── Exit check 1: TIME EXIT ───────────────────────────────────────────────
    try:
        entry_time = datetime.strptime(trade["Entry_Time"], "%Y-%m-%d %H:%M:%S")
        if datetime.now() >= entry_time + timedelta(minutes=TIME_EXIT_MINS):
            log.info("[Trail] TIME EXIT for %s (entry=%s)", symbol, entry_time)
            # Cancel both GTT legs
            if trade.get("GTT_Target_ID"):
                broker.cancel_gtt(trade["GTT_Target_ID"], symbol, token)
            if gtt_sl_id:
                broker.cancel_gtt(gtt_sl_id, symbol, token)
            # Market sell
            broker.place_sell_market_order(symbol, token, qty)
            close_trade(order_id, "TIME_EXIT")
            return
    except (ValueError, TypeError):
        pass   # Entry_Time parse error — skip time exit

    # ── Exit check 2: TARGET HIT ──────────────────────────────────────────────
    if ltp >= target:
        log.info("[Trail] TARGET HIT %s ltp=%.2f tgt=%.2f", symbol, ltp, target)
        close_trade(order_id, "TARGET_HIT")
        update_trade(order_id, {"SubAction":"TARGET_HIT"})
        return

    # ── Exit check 3: SL HIT ─────────────────────────────────────────────────
    if ltp <= last_sl:
        log.info("[Trail] SL HIT %s ltp=%.2f sl=%.2f", symbol, ltp, last_sl)
        close_trade(order_id, "SL_HIT")
        update_trade(order_id, {"SubAction":"SL_HIT"})
        return

    # ── Phase 1: BREAKEVEN ────────────────────────────────────────────────────
    # Move SL to entry when LTP is ≥ 1% above entry AND SL is still below entry
    if ltp >= entry * (1 + BREAKEVEN_PCT) and last_sl < entry:
        new_sl = round(entry, 2)
        log.info("[Trail] BREAKEVEN %s SL → %.2f", symbol, new_sl)
        _apply_sl_update(broker, order_id, symbol, token, qty,
                         gtt_sl_id, new_sl, "BREAKEVEN")
        return

    # ── Phase 2: TRAILING ─────────────────────────────────────────────────────
    step         = calc_step(ltp)
    candidate_sl = round(ltp - step, 2)

    if candidate_sl > last_sl:
        log.info("[Trail] TRAIL %s ltp=%.2f step=%.2f sl %.2f → %.2f",
                 symbol, ltp, step, last_sl, candidate_sl)
        _apply_sl_update(broker, order_id, symbol, token, qty,
                         gtt_sl_id, candidate_sl, "TRAILING")
    else:
        log.debug("[Trail] %s ltp=%.2f candidate_sl=%.2f not better than last_sl=%.2f — skip",
                  symbol, ltp, candidate_sl, last_sl)


def _apply_sl_update(
    broker,
    order_id:  str,
    symbol:    str,
    token:     str,
    qty:       int,
    gtt_sl_id: str,
    new_sl:    float,
    sub_action: str,
) -> None:
    """
    1. Modify GTT SL rule via Angel API.
    2. Update Last_SL + SubAction + Trailing_Active in S3 CSV.
    """
    # Modify GTT SL if we have a GTT ID
    if gtt_sl_id:
        result = broker.modify_gtt_sl(gtt_sl_id, symbol, token, qty, new_sl)
        if result["status"] != "success":
            log.warning("[Trail] GTT modify failed for %s: %s",
                        symbol, result.get("message"))
            # Still update CSV — SL state is source of truth
    else:
        log.warning("[Trail] No GTT SL ID for %s — updating CSV only", symbol)

    # Update CSV
    update_trailing_sl(order_id, new_sl)
    update_trade(order_id, {"SubAction": sub_action, "Trailing_Active":"True"})


# ── Background thread ─────────────────────────────────────────────────────────

class TrailingEngine:
    """
    Daemon thread that runs process_trade() for every ACTIVE trade.
    """
    def __init__(self) -> None:
        self.running = False
        self._thread: Optional[threading.Thread] = None
        self.status  = {"last_run":None,"active_trades":0,"errors":[]}

    def start(self) -> None:
        if self.running: return
        self.running = True
        self._thread = threading.Thread(
            target=self._loop, name="trailing-engine", daemon=True)
        self._thread.start()
        log.info("[Trail] Engine started (poll=%ds)", TRAIL_POLL_SECS)

    def stop(self) -> None:
        self.running = False

    def _loop(self) -> None:
        while self.running:
            try:
                self._run_cycle()
            except Exception as e:
                log.error("[Trail] Cycle error: %s", e, exc_info=True)
                self.status["errors"].append(str(e))
            time.sleep(TRAIL_POLL_SECS)

    def _run_cycle(self) -> None:
        if not _market_open():
            return
        from angel_broker import get_broker
        broker = get_broker()
        trades = load_active()
        self.status["last_run"]     = datetime.now().isoformat()
        self.status["active_trades"] = len(trades)
        for trade in trades:
            try:
                process_trade(broker, trade)
            except Exception as e:
                log.error("[Trail] Error processing %s: %s",
                          trade.get("Symbol","?"), e)
        log.debug("[Trail] Cycle done — %d active trades", len(trades))


_engine = TrailingEngine()

def get_engine()   -> TrailingEngine: return _engine
def start_engine() -> None:           _engine.start()
