...
class AngelBroker:
    ...
    def place_gtt_order(
        ...
        """
        Create two GTT rules:
          Leg 1 — target SELL at target_price
          Leg 2 — SL SELL    at sl_price (limit = sl*0.995 for slippage)
        Returns {"status":"success","target_gtt_id":"...","sl_gtt_id":"..."}
        """
        product_type = self.get_product_type(trading_symbol)
        base = {
            "tradingsymbol":trading_symbol,"symboltoken":str(token),
            "exchange":exchange,"producttype":product_type,
            "transactiontype":"SELL","qty":str(qty),
            "disclosedqty":str(qty),"timeperiod":str(GTT_DAYS),
        }
        try:
            tgt_p = {**base,"triggerprice":str(round(target_price,2)),
                     "price":str(round(target_price,2))}
            sl_p  = {**base,"triggerprice":str(round(sl_price,2)),
                     "price":str(round(sl_price*0.995,2))}
            t_r   = self._call(self._obj.gttCreateRule, tgt_p)
            s_r   = self._call(self._obj.gttCreateRule, sl_p)
            t_id  = str((t_r.get("data") or {}).get("id",""))
            s_id  = str((s_r.get("data") or {}).get("id",""))
            log.info("[GTT] Created — target_id=%s sl_id=%s", t_id, s_id)
            return {"status":"success","target_gtt_id":t_id,"sl_gtt_id":s_id}
        except Exception as e:
            log.warning("[GTT] Create failed: %s", e)
    ...
    def modify_gtt_sl(
        ...
        """
        Modify an existing SL GTT rule to a new (higher) stop-loss price.
        Called by the trailing engine on every upward SL step.
        """
        product_type = self.get_product_type(trading_symbol)
        params = {
            "id":          str(gtt_id),
            ...
            "timeperiod":  str(GTT_DAYS),
            "triggerprice":str(round(new_sl,2)),
            "price":       str(round(new_sl*0.995,2)),
        }
        try:
            r = self._call(self._obj.gttModifyRule, params)
            if r.get("status"):
                log.info("[GTT] Modified SL gtt_id=%s → %.2f", gtt_id, new_sl)
                return {"status":"success"}
            log.warning("[GTT] Modify failed: %s", r.get("message"))
            return {"status":"error","message":r.get("message","")}
        except Exception as e:
            log.warning("[GTT] Modify exception: %s", e)
            return {"status":"error","message":str(e)}
